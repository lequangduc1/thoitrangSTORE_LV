    ...
    <div class="container-fluid">
        <div class="row">
            <div class="card">
                ...
                @endif
                <div class="card-body">
                    <form class="form-horizontal form-material" action="{{route('admin.products.store')}}" method="POST"
                            ...
                            <div class="col-sm-12 border-bottom">
                                <select name="idsanpham" class="form-select shadow-none p-0 border-0 form-control-line"
                                    ...
                                    @foreach($productMaster as $value)
                                        <option
                                            value="{{$value->id}}" {{ (int)$products->idsanpham == (int)$value->id ? 'selected' : ''}}>{{$value->ten_sp}}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        <div class="form-group mb-4">
                            <label class="col-md-12 p-0"><b>Ảnh sản phẩm</b><span
                                    class="input__required">*</span></label>
                            <div class="col-md-12">
                                <input type="file" class="form-control p-0 border-0" name="anhsanpham"
                                       accept="image/png, image/jpeg" id="image"/>
                            </div>
                            <div class="col-md-12 d-flex justify-content-center border-bottom p-1 m-1">
                                <img
                                    src="{{ ($products->anhsanpham != '') ? URL::to('/').'/'.$products->anhsanpham : ''; }}"
                                    style="max-height: 400px;" id="previewImage"/>
                            </div>
                        </div>
                        <div class="form-group mb-4 col-sm-6" style="float: left">
                            <label class="col-md-12 p-0"><b>Giá sản phẩm</b><span
                                    class="input__required">*</span></label>
                            <div class="col-md-12 border-bottom p-0">
                                <input type="text" class="form-control p-0 border-0" name="giasanpham"
                                       value="{{$products->giasanpham}}" required/>
                            </div>
                        </div>
                        <div class="form-group mb-4 col-sm-6" style="float: left">
                            <label class="col-md-12 p-0"><b>Số lượng</b><span class="input__required">*</span></label>
                            <div class="col-md-12 border-bottom p-0">
                                <input type="text" class="form-control p-0 border-0" name="soluong"
                                       value="{{$products->soluong}}" required></div>
                        </div>
                            ...
                            <div class="col-sm-12 border-bottom">
                                <select name="idmau" class="form-select shadow-none p-0 border-0 form-control-line">
                                    @foreach($productColor as $value)
                                        <option
                                            value="{{$value->id}}" {{ (int)$products->idmau ==  (int)$value->id ? 'selected' : ''}}>{{$value->tenmau}}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        <div class="form-group col-sm-6 mb-4" style="float: left">
                            <label class="col-sm-12"><b>Kích thước sản phẩm</b><span
                                    class="input__required">*</span></label>
                            <div class="col-sm-12 border-bottom">
                                <select name="idsize" class="form-select shadow-none p-0 border-0 form-control-line">
                                    @foreach($productSize as $value)
                                        <option
                                            value="{{$value->id}}" {{ (int)$products->idsize ==  (int)$value->id ? 'selected' : ''}}>{{$value->tensize}}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
